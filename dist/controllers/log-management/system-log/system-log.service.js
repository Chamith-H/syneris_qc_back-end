...
let SystemLogService = class SystemLogService {
    constructor(systemLogModel) {
        this.systemLogModel = systemLogModel;
    }
    async add_toSystemLog(dataModel, dto) {
        const newData = dataModel(dto.data);
        const response = await newData.save();
        if (!response) {
            throw new common_1.BadRequestException(dto.errorMessage);
        }
        const systemLog = {
            origin: response._id,
            target: dto.target,
...
exports.SystemLogService = SystemLogService = __decorate([
    ...
    __param(0, (0, mongoose_1.InjectModel)(system_log_schema_1.SystemLog.name)),
